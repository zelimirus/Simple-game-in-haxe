...
class Main extends Application {
    ...
    var message = new Text(Config.welcomeMessage, {font: 'bold 15px Arial',fill: '#ff0000',align: 'center'});
    ...
    function _init() {
        ...
        imgButton.on('mousedown', startAnimation);
    ...
    function draw() {
        ...
        } else if (gameStatus == gameStateCHECK_WIN) return;
    ...
    function onWinAmin() {
        ...
        if (isAnimated) {
            var wabbitTexture = Texture.fromImage("assets/images/dolar2.png");
            var count = startDolarCount;
            var container = new Container();
            stage.addChild(container);

            var money1 = new Texture(wabbitTexture.baseTexture);
            var money2 = new Texture(wabbitTexture.baseTexture);
            var money3 = new Texture(wabbitTexture.baseTexture);
            var money4 = new Texture(wabbitTexture.baseTexture);
            var money5 = new Texture(wabbitTexture.baseTexture);
            var moneyTextures = [money1, money2, money3, money4, money5];
            var moneyType = 3;
            var currentTexture = moneyTextures[moneyType];
            var i = 0;
            var money = new Money(currentTexture);
            while (startDolarCount > i) {
            ...
            for (i in 0...dolar.length) {
                ...
                if (money.position.y > maxY) {
                    ...
                    if (Math.random() > 0.5) money.speedY -= Math.random() * 6;
            ...
            if (isAnimated) {
                Browser.window.requestAnimationFrame(cast onWinAmin);
            }
